if __name__ == '__main__':
    ...
    data_file = Class_FileManager.FileManager('clean_data.csv')
    ...
    from Classes import Class_Search
    searcher_one = Class_Search.Search(number_of_stops, departure_am_pm, maximum_price, clear_data_frame=df)
    print(searcher_one)
    ...
    filtered_data_file = Class_FileManager.FileManager()
    ...
    data_file = Class_FileManager.FileManager('searcher_one.csv')
    df2 = data_file.open_data()

    # Let's create some plots
    from Classes import Class_DataDescriptor
    describe_df2 = Class_DataDescriptor.DataDescriptor(df=df2)
